...
class ContactForm(forms.ModelForm):
    honeypot = forms.CharField(
        required=False,
        widget=forms.HiddenInput(attrs={'class': 'honeypot'})
    )
    timestamp = forms.CharField(
        required=False,
        widget=forms.HiddenInput(attrs={'class': 'timestamp'})
    )

    class Meta:
        model = ContactRequest
        fields = [
            'request_type', 'name', 'email', 'subject', 'message',
            'consent_personal_data', 'consent_newsletter'
        ]
        labels = {
        ...
        widgets = {
            'request_type': forms.Select(attrs={
                'class': 'form-select',
                'help-text': 'Выберите тип обращения'
            }),
            'subject': forms.Textarea(attrs={
                'rows': 2,
                'class': 'form-control',
                'help-text': 'Введите тему сообщения'
            }),
            'message': forms.Textarea(attrs={
                'rows': 10,
                'class': 'form-control',
                'help-text': 'Опишите вашу проблему или вопрос подробно'
            }),
            'name': forms.TextInput(attrs={
                'class': 'form-control',
                'help-text': 'Ваше имя (необязательно)'
            }),
            'email': forms.EmailInput(attrs={
                'class': 'form-control',
                'help-text': 'Введите ваш email для обратной связи'
            }),
            'consent_personal_data': forms.CheckboxInput(attrs={
                'class': 'form-check-input',
                'help-text': 'Поставив галочку в этом вопросе вы даете согласие на обработку ваших персональных данных'
            }),
            'consent_newsletter': forms.CheckboxInput(attrs={
                'class': 'form-check-input',
