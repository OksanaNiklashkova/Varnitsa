...
class Publication(models.Model):
    ...
    preview = models.ImageField(
        upload_to="images/blog/preview/", default="images/placeholder.png", verbose_name="Превью"
    )
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Дата создания")
    is_published = models.BooleanField(default=True, verbose_name="Признак публикации")
    views_counter = models.IntegerField(default=0, verbose_name="Счетчик просмотров")
    publication_type = models.CharField(
        max_length=10, choices=PUBLICATION_TYPES, default="full", verbose_name="Тип статьи"
    )
...
class Photo(models.Model):
    publication = models.ForeignKey(
        Publication, on_delete=models.CASCADE, related_name="photos", verbose_name="Статья"
    )
    ...
    def save(self, *args, **kwargs):
        if not self.pk:
            ...
            max_order = (
                Photo.objects.filter(publication=self.publication).aggregate(models.Max("order"))["order__max"] or 0
            )
